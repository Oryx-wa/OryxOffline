...
namespace Afex.WarehouseMan.Tests
{
    public abstract class SampleLTETestBase : AbpIntegratedTestBase<SampleLTETestModule>
    {
        ...
        protected void UsingDbContext(Action<SampleLTEDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SampleLTEDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SampleLTEDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SampleLTEDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SampleLTEDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SampleLTEDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SampleLTEDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SampleLTEDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SampleLTEDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SampleLTEDbContext, Task<T>> func)
        {
            T result;
